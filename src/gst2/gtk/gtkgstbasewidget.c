...
GST_DEBUG_CATEGORY (gst_debug_gtk_base_custom_widget);
#define GST_CAT_DEFAULT gst_debug_gtk_base_custom_widget
...
static gboolean
_calculate_par (GtkGstBaseCustomWidget * widget, GstVideoInfo * info)
{
  gboolean ok;
  gint width, height;
  gint par_n, par_d;
...
static void
_apply_par (GtkGstBaseCustomWidget * widget)
{
...
static gboolean
_queue_draw (GtkGstBaseCustomWidget * widget)
{
  GTK_GST_BASE_CUSTOM_WIDGET_LOCK (widget);
  widget->draw_id = 0;
  ...
  GTK_GST_BASE_CUSTOM_WIDGET_UNLOCK (widget);

  return G_SOURCE_REMOVE;
}

static void
_update_par (GtkGstBaseCustomWidget * widget)
{
  GTK_GST_BASE_CUSTOM_WIDGET_LOCK (widget);
  if (widget->pending_resize) {
    GTK_GST_BASE_CUSTOM_WIDGET_UNLOCK (widget);
...
static void
gtk_gst_base_custom_widget_get_preferred_width (GtkWidget * widget, gint * min,
    gint * natural)
{
  GtkGstBaseCustomWidget *gst_widget = (GtkGstBaseCustomWidget *) widget;
...
static void
gtk_gst_base_custom_widget_get_preferred_height (GtkWidget * widget, gint * min,
    gint * natural)
{
  GtkGstBaseCustomWidget *gst_widget = (GtkGstBaseCustomWidget *) widget;
...
static void
gtk_gst_base_custom_widget_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  GtkGstBaseCustomWidget *gtk_widget = GTK_GST_BASE_CUSTOM_WIDGET (object);
...
static void
gtk_gst_base_custom_widget_get_property (GObject * object, guint prop_id,
...
static void
_fit_stream_to_allocated_size (GtkGstBaseCustomWidget * base_widget,
...
void
gtk_gst_base_custom_widget_display_size_to_stream_size (GtkGstBaseCustomWidget * base_widget,
...
gtk_gst_base_custom_widget_scroll_event (GtkWidget * widget, GdkEventScroll * event)
{
  ...
  if ((element = g_weak_ref_get (&base_widget->element))) {
    if (GST_IS_NAVIGATION (element)) {
      ...
      gtk_gst_base_custom_widget_display_size_to_stream_size (base_widget, event->x,
          event->y, &x, &y);
...
void
gtk_gst_base_custom_widget_class_init (GtkGstBaseCustomWidgetClass * klass)
{
  GObjectClass *gobject_klass = (GObjectClass *) klass;
  GtkWidgetClass *widget_klass = (GtkWidgetClass *) klass;

  gobject_klass->set_property = gtk_gst_base_custom_widget_set_property;
  gobject_klass->get_property = gtk_gst_base_custom_widget_get_property;
  ...
  widget_klass->get_preferred_width = gtk_gst_base_custom_widget_get_preferred_width;
  widget_klass->get_preferred_height = gtk_gst_base_custom_widget_get_preferred_height;
  widget_klass->key_press_event = gtk_gst_base_custom_widget_key_event;
  widget_klass->key_release_event = gtk_gst_base_custom_widget_key_event;
  widget_klass->button_press_event = gtk_gst_base_custom_widget_button_event;
  widget_klass->button_release_event = gtk_gst_base_custom_widget_button_event;
  widget_klass->motion_notify_event = gtk_gst_base_custom_widget_motion_event;
  widget_klass->scroll_event = gtk_gst_base_custom_widget_scroll_event;
  widget_klass->touch_event = gtk_gst_base_custom_widget_touch_event;

  GST_DEBUG_CATEGORY_INIT (gst_debug_gtk_base_custom_widget, "gtkbasecustomwidget", 0,
      "Gtk Video Base Custom Widget");
}

void
gtk_gst_base_custom_widget_init (GtkGstBaseCustomWidget * widget)
{
  int event_mask;
...
void
gtk_gst_base_custom_widget_finalize (GObject * object)
{
  GtkGstBaseCustomWidget *widget = GTK_GST_BASE_CUSTOM_WIDGET (object);
...
void
gtk_gst_base_custom_widget_set_element (GtkGstBaseCustomWidget * widget,
    GstElement * element)
{
...
gboolean
gtk_gst_base_custom_widget_set_format (GtkGstBaseCustomWidget * widget,
    GstVideoInfo * v_info)
{
  GTK_GST_BASE_CUSTOM_WIDGET_LOCK (widget);
...
void
gtk_gst_base_custom_widget_set_buffer (GtkGstBaseCustomWidget * widget, GstBuffer * buffer)
{
...
void
gtk_gst_base_custom_widget_queue_draw (GtkGstBaseCustomWidget * widget)
{
  /* As we have no type, this is better then no check */
  g_return_if_fail (GTK_IS_WIDGET (widget));
